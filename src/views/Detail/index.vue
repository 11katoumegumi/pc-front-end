      ...
      <div class="mainCon">
        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              <InputNumber v-model="count" :max="max" :min="min" />
              <div class="add">
                <a href="javascript:" @click="handleAddCartClick">加入购物车</a>
              </div>
            </div>
          </div>
        </div>
...
<script>
import { reqGetGoodsDetail, reqAddToCart } from "../../api/detail";
...
export default {
  ...
  computed: { ...mapState("imageList", ["imageUrl"]) },
  components: {
    ImageList,
    Zoom,
    InputNumber,
  ...
  methods: {
    ...mapActions("imageList", ["changeImage"]),
    handleAttrValueClick(saleAttrValue, spuSaleAttrValueList) {
  ...
  async mounted() {
    const resdata = await reqGetGoodsDetail(this.$route.params.id);
    this.categoryView = resdata.categoryView;
    this.skuInfo = resdata.skuInfo;
    this.spuSaleAttrList = resdata.spuSaleAttrList;
    this.changeImage(this.skuInfo.skuDefaultImg);
  },
};
</script>
