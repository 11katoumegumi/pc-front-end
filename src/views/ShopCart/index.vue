        ...
        >
          <li class="cart-list-con1">
            <input
              ...
              @click="
                handleCheckBoxClick(cartInfo, $event.target.checked ? 1 : 0)
              "
            />
          </li>
          ...
          <li class="cart-list-con6">
            <span class="sum">{{ cartInfo.skuPrice }}</span>
          </li>
          ...
          <li class="cart-list-con4">
            <span class="price">{{ cartInfo.skuPrice * cartInfo.skuNum }}</span>
          </li>
          <li class="cart-list-con7">
            <a
              href="#none"
              class="sindelet"
              @click="handleDeleteClick(cartInfo)"
              >删除</a
            >
            <br />
            <a href="#none">移到收藏</a>
    ...
    <div class="cart-tool">
      <div class="select-all">
        <input
          ...
          @click="handleAllCheckedClick($event.target.checked)"
        />
        <span>全选</span>
      </div>
      <div class="option">
        <a href="#none" @click="handleDeleteManyClick">删除选中的商品</a>
...
<script>
import { reqGetCartList, reqCheckCart, deleteCart } from "../../api/shopcart";
export default {
  name: "ShopCart",
  data() {
    return {
  ...
  methods: {
    async handleCheckBoxClick(item, isChecked) {
      item.isChecked = isChecked;
      await reqCheckCart(item.skuId, isChecked);
    },
    ...
    handleAllCheckedClick(isChecked) {
      this.cartInfoList.forEach((checkitem) => {
        this.handleCheckBoxClick(checkitem.skuId, isChecked);
      });
    },
    async getCartList() {
